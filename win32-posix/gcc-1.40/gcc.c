   ...
   "cpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} %{i*} %{trigraphs} -undef \
    ...
    %{!M*:%{!E:cc1 %{!pipe:%g.cpp} %1 \
		   ...
		   %{S:%W{o*}%{!o*:-o %b.s}}%{!S:-o %{|!pipe:%g.s}} |\n\
              %{!S:a386 %{R} %{j} %{J} %{h} %{d2} %a %{gg:-G %g.sym}\
...
if(ret_code!=0) {
	      Xfatal ("Program %s got fatal signal %d.", command, (char*)(ret_code & 0x7F));
	      fflush (stderr);
		exit(-1);
		}
      fflush (stderr);
	...
	if (status != 0)
	  {
	    ...
	    if ((status & 0x7F) != 0)
	      Xfatal ("Program %s got fatal signal %d.", prog, (char*)(status & 0x7F));
...
printf("ret_code is %d\n",ret_code);
	if(ret_code!=0) {
printf("ret_code is not zero\n");
	      Xfatal ("Program %s got fatal signal %d.", prog, (char*)(ret_code & 0x7F));
	}
    return ret_code;
  }
#endif
     ...
     int inswitch;
{
  ...
  while (c = *p++)
	...
	switch (c = *p++)
	  {
	  case 0:
	    Xfatal ("Invalid specification!  Bug in cc.");
     ...
     char **argv;
{
  ...
  if (n_infiles == 0)
    Xfatal ("No input files specified.");
     ...
     int size;
{
  ...
  if (value == 0)
    Xfatal ("virtual memory exhausted");
  return value;
}
