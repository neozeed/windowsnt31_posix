...
char *version_string = "GNU sed version 1.14b";
  ...
  while ((ch = inchar ()) != EOF && (ch != slash || in_char_class))
      ...
      else if (ch == ']')
	{
	  add1_buffer (b, ch);
	  {
	    ...
	    int len = size_buffer (b);
	    /* In regexp syntax, the close first brace doesn't conclude
	     * either character set: "[]xyz]" or "[^]abc]".  We know
	     * we've read at least "[" and "]" so len >= 2.
	     */
	    if (   (regexp[len - 2] != '[')
		&& (   (len == 2)
		    || (regexp[len - 2] != '^')
