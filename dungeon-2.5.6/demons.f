...
C NOTHING ELSE.
C
2050         IF(ZAND(OFLAG2(OBJ),FITEBT).EQ.0) GO TO 2100
         VOPPS(I)=OBJ 
...
C OF FIGHTING.
         OFLAG2(OBJ)=ZOR(OFLAG2(OBJ),FITEBT)
         VOPPS(I)=OBJ 
C SET UP OPP.
         GO TO 2400
C
2200         IF((ZAND(OFLAG2(OBJ),FITEBT).EQ.0).OR.(RA.EQ.0))
     1       GO TO 2300 
C JOTHING TK DO.
         PRSA=FIGHTW 
C HAVE A FIGHT.
...
C TURN OFF ENGROSSED.
         AFLAG(PLAYER)=ZAND(AFLAG(PLAYER),NOT(ASTAG))
         OFLAG2(OBJ)=ZAND(OFLAG2(OBJ), NOT(STAGBT+FITEBT))
...
C
       PBLOSE=10 
C BAD LK PROB.
       OFLAG2(V)=ZOR(OFLAG2(V),FITEBT) 
C YES, VILLAIN GETS MAD.
       IF(ZAND(AFLAG(H),ASTAG).EQ.0) GO TO 100 
C HERO STAG?
       CALL RSPEAK(591) 
C YES, CANT FIGHT.
       AFLAG(H)=ZAND(AFLAG(H), NOT(ASTAG))
       RETURN
C
...
C ASSUME NO WEAPON.
       DO 200 I=1,OLNT 
C SEARCH VILLAIN.
         IF((OCAN(I).EQ.V).AND.(ZAND(OFLAG2(I),WEAPBT).NE.0))
     1       DWEAP=I
200       CONTINUE
...
C BAD LK PROB.
       AFLAG(H)=ZAND(AFLAG(H),NOT(ASTAG)) 
C VILL STRIKING.
       IF(ZAND(OFLAG2(V),STAGBT).EQ.0) GO TO 1200 
C VILL STAGGERED?
       OFLAG2(V)=ZAND(OFLAG2(V), NOT(STAGBT)) 
C MAKE HIM OK.
       CALL RSPSUB(594,DV) 
C DESCRIBE.
       RETURN
...
C CHOOSE TABLE ENTRY.
       ...
       I=(MOD(MI,1000)+ZRND(MI/1000))+MBASE+1
       J=DV
...
C STAGGERED.
       AFLAG(H)=ZOR(AFLAG(H),ASTAG)
       GO TO 4000
C
3550       OFLAG2(V)=ZOR(OFLAG2(V),STAGBT)
       GO TO 4000
C
...
C DEAD?
       OFLAG2(V)=ZAND(OFLAG2(V), NOT(FITEBT)) 
C YES, NOT FIGHTING.
       CALL RSPSUB(572,DV) 
C HE DIES.
