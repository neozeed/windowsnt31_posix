...
#define XPFILE "/usr/local/lib/bison.simple"
#endif
#ifndef XPFILE1
//#define XPFILE1 "./bison.hairy"
#define XPFILE1 "/usr/local/lib/bison.simple"
#endif
...
openfiles()
{
  ...
  if (filename == 0 && cp != 0)
    {
      filename = malloc(strlen(cp) + strlen(XPFILE) + 2);
      strcpy(filename, cp);
...
#endif /* MSDOS */
  fparser = tryopen(filename ? filename : XPFILE, "r");
